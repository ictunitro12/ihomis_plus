					...
					<div id="baseDateControl">
						...
						<!-- <div class="row">
							<div id="yearly" class="col-md-12">
								...
								<input type="number" min="1970" max="<?php echo date("Y"); ?>" value="<?php echo date("Y"); ?>" class="form-control form-control-mg" placeholder="yyyy" name="filter" id="qyear" autocomplete="off"/>
							</div>
						</div> -->
					</div>
				</div>
...
<form id="frmLCauseErRep" name="frmLCauseRep" method="post">
	...
	$('#filter').on('click', function() {
		if ($("#monthly").prop('checked') == true) {
			if ($("#mnth").val() == "" || $("#mnth").val() == null || $("#mnth").val() == undefined) {
				...
				var src = (baseURL + "Rep_ER/LCausesER/monthly/" + $("#mnth").val());
			...
			if ($("#squarter").val() == "" || $("#squarter").val() == null || $("#squarter").val() == undefined) {
				...
				if ($("#qyear").val() == "" || $("#qyear").val() == null || $("#qyear").val() == undefined) {
					...
					var year = $("#qyear").val();
					var src = (baseURL + "Rep_ER/LCausesER/quarterly/" + $("#squarter").val() + year);
					var x = document.getElementById("CSS_pdf").src = src;
					$("#CSS_pdf").attr("src", src);
					$('#DateSearch').modal('hide');
				}
			...
			if ($("#qyear").val() == "" || $("#qyear").val() == null || $("#qyear").val() == undefined) {
				...
				var year = $("#qyear").val();
				/* $('#ModalCSS').modal('show'); */
				var src = (baseURL + "Rep_ER/LCausesER/yearly/" + year);
				$("#CSS_pdf").attr("src", src);
				$('#DateSearch').modal('hide');
			}
		}
