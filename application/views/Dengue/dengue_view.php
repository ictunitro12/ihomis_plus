...
<div class="card card-accent-success">
		...
		<div class="row">
			...
			<div class="col-md-6 col-md-auto">
				<div class="btn-group pull-right" role="group" aria-label="">
					<a href="<?php echo site_url('Dengue/PatientForm'); ?>" data-toggle="tooltip" title="Register Patient" class="btn btn-success btn-md  btn-ladda" data-style="zoom-in"><i class="fa fa-user-plus"></i>
					</a>
	...
	<div class="card-body table-responsive">
		<table id="DengueTable" class="table table-sm table-striped table-bordered table-condensed table-hover" width="100%">
...
<form id="frmDenguePDF">
	<div class="modal fade" id="ModalDengue" tabindex="-1" role="dialog">
		<div class="modal-dialog modal-xl" role="dialog">
			<div class="modal-content">
				<div class="modal-body">
					<h3 class="modal-title text-success"><i class="fa fa-heartbeat"></i> Dengue</h3>
					<iframe id="DenguePDF" type="application/pdf" frameborder="2" width="100%" height="800px"></iframe>
				</div>
			</div>
		</div>
	</div>
...
<script src="<?php echo base_url() ?>assets/scripts/dengue/delete.js"></script>
...
<script type="text/javascript">
	...
	$("#DengueTable").on("click", ".ModalDeleteDengue", function() {
		var data = $(this).data();
		console.log(data);
		$('#DeleteDengue').modal('show');
		$("#formIdentification").val('delete');
		$("#deletecode").val(data['enccode']);
	});

	/*$('#DengueTable').on('click','.ModalPatientInfo',function(){
	...
	$('#btnCSS').on('click', function(data) {
		$('#ModalDengue').modal('show');
		/* var from = $.session.get('from');
		var to = $.session.get('to');
		var src = baseURL+'Dengue/DengueForm/'+ from + "/" + to; */
		var src = baseURL + 'Dengue/DengueForm';
		var x = document.getElementById("DenguePDF").src = src;
		$("#DenguePDF").attr("src", src);
	});

	$('#ModalDengue').on('hidden.coreui.modal', function() {
		$("#DenguePDF").removeAttr("src").attr("src", "");
	});
</script>