					...
					<div id="baseDateControl">
						<div class="row">
							...
							<div class="col-md-4">
								...
								<label for="quarterly" align="center" required>
									<h5>&nbsp; Quarterly</h5>
								</label>
							</div>
							<div class="col-md-4">
								<input type="radio" id="annual" name="filter" value="annual">
								<label for="annual" required>
									<h5>&nbsp; Annual</h5>
								</label>
							</div>
	...
	$('#filter').on('click', function() {
		if ($("#monthly").prop('checked') == true) {
			if ($("#mnth").val() == "" || $("#mnth").val() == null || $("#mnth").val() == undefined) {
				...
				var src = (baseURL + "Rep_Outpatient/LCauses/monthly/" + $("#mnth").val());
			...
			if ($("#squarter").val() == "" || $("#squarter").val() == null || $("#squarter").val() == undefined) {
				...
				if ($("#qyear").val() == "" || $("#qyear").val() == null || $("#qyear").val() == undefined) {
					...
					var year = $("#qyear").val();
					var src = (baseURL + "Rep_Outpatient/LCauses/quarterly/" + $("#squarter").val() + year);
					var x = document.getElementById("CSS_pdf").src = src;
					$("#CSS_pdf").attr("src", src);
					$('#DateSearch').modal('hide');
					/* $('#ModalCSS').modal('show'); */
			...
			if ($("#qyear").val() == "" || $("#qyear").val() == null || $("#qyear").val() == undefined) {
				...
				var year = $("#qyear").val();
				/* $('#ModalCSS').modal('show'); */
				var src = (baseURL + "Rep_Outpatient/LCauses/yearly/" + year);
				$("#CSS_pdf").attr("src", src);
				$('#DateSearch').modal('hide');
			}
		}
