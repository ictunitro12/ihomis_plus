		...
		<div class="modal-dialog modal-dialog-centered modal-lg" role="dialog">
			<div class="modal-content card-accent-success">
				<div class="modal-body border-success">
					<h1 class="modal-title text-success"><i class="fa fa-search-plus"></i> <span id="addclaimheader">New Claim</span></h1>
					<ul class="list-group">
		...
		<div class="modal-dialog modal-dialog-centered modal-md" role="dialog">
			<div class="modal-content">
				<div class="modal-header bg-danger">
					<h5 class="modal-title text-white"><i class="fa fa-user-o"></i>&nbsp <span id="delpatname"></span></h5>
...
<script type="text/javascript">
			...
			$.ajax({
				...
				success: function(data, response) {
					if(data.status == 'success'){
						...
						$('.pbfebBody').html('<object id="pdfframe" type="application/pdf" data="data:application/pdf;base64,' + data.data.pdf_base64 + '" width="100%" style="height:100%; overflow:hidden;"></object>');
						Claimlist(phicnum);
					}else{
	...
	function PatVisitList(data, patientname) {
		...
		$('#visitLogList').on('dblclick', 'tr', function() {
			...
			$.get(baseURL + "Philhealth/PHICAccreditations", function(data) {
				for (i in data) {
					$("#paccreno").append("<option value='" + data[i].paccreno + "'>" + data[i].claimtype + "</option>");
				}
			}, "json");
		...
		$.get(baseURI + "Philhealth/PHICAccreditations", function(data) {
			for (i in data) {
				if (accreno == data[i].paccreno) {
					$("#paccreno").append("<option value='" + data[i].paccreno + "' selected>" + data[i].claimtype + "</option>");
				} else {
					$("#paccreno").append("<option value='" + data[i].paccreno + "'>" + data[i].claimtype + "</option>");
				}
	...
	$('#ClaimsTable').on('click', '.ModalClaimCF3', function() {
		...
		window.location.href = baseURI + "Philhealth/claimForm3View?enccode=" + enccodeURI + '&accreno=' + accrenoURI;
	});
	...
	$('#ClaimsTable').on('click', '.ModalClaimCF4', function() {
		...
		window.location.href = baseURI + "Philhealth/claimForm4View?enccode=" + enccodeURI + '&accreno=' + accrenoURI;
	});
	...
	function setVerifyInformation(enccode){
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				membdate = ((membdate.getMonth() > 8) ? (membdate.getMonth() + 1) : ('0' + (membdate.getMonth() + 1))) + '/' + ((membdate.getDate() > 9) ? membdate.getDate() : ('0' + membdate.getDate())) + '/' + membdate.getFullYear();
				...
				patbdate = ((patbdate.getMonth() > 8) ? (patbdate.getMonth() + 1) : ('0' + (patbdate.getMonth() + 1))) + '/' + ((patbdate.getDate() > 9) ? patbdate.getDate() : ('0' + patbdate.getDate())) + '/' + patbdate.getFullYear();
				...
				admdate = ((admdate.getMonth() > 8) ? (admdate.getMonth() + 1) : ('0' + (admdate.getMonth() + 1))) + '/' + ((admdate.getDate() > 9) ? admdate.getDate() : ('0' + admdate.getDate())) + '/' + admdate.getFullYear();
				$("#verfiy_admdate").text(admdate);

				var disdate = new Date(pbefdata.disdate);
				disdate = ((disdate.getMonth() > 8) ? (disdate.getMonth() + 1) : ('0' + (disdate.getMonth() + 1))) + '/' + ((disdate.getDate() > 9) ? disdate.getDate() : ('0' + disdate.getDate())) + '/' + disdate.getFullYear();
				...
				var phic = parseFloat(pbefdata.amt1) + parseFloat(pbefdata.amt2);
