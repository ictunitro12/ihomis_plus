			...
			<div class="row">
				<div class="col-md-6 col-md-auto">
					<div class="h3 text text-success">
						<h4 class="modal-title text-success"><i class="fa fa-address-card-o" aria-hidden="true"></i>&nbsp;Medical Certificate ER</h4>
					</div>
				</div>
				<div class="col-md-6 col-md-auto">
...
<script type="text/javascript">
	...
	function PatSearchList(data) {
		...
		$('#TblPatSearchList').on('click', 'tr', function() {
			...
			$.ajax({
				type: "POST",
				url: baseURL + "Rep_MedicalRecord/ERHisto/" + data['hpercode'],
				data: "JSON",
				processData: false,
				contentType: false,
				cache: false,
				...
				$.ajax({
					...
					success: function(data) {
						if ($('#formIden').val() == 'insert') {
							...
							$('#PatadmitList').modal('hide');
							var x = document.getElementById("pdf_patrec").src = baseURL + "Rep_MedicalRecord_pdf/MedicalCertER_pdf/" + encodeURIComponent(encodeURIComponent(enccode));
	...
	$('#frmPatSearch').submit('click', function() {
		$.ajax({
			url: baseURL + "Rep_MedicalRecord/PatientSearchER",
			type: "POST",
