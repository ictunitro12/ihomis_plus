...
table#IliTable.dataTable tbody tr:hover {
...
<div class="card card-accent-success">
		...
		<div class="row">
			...
			<div class="col-md-6 col-md-auto">
				<div class="btn-group pull-right" role="group" aria-label="">
					<a href="<?php echo site_url('PIDSR_ILI/PatientForm');?>" data-toggle="tooltip" title="Register Patient" class="btn btn-success btn-md  btn-ladda" data-style="zoom-in"><i class="fa fa-user-plus"></i>
					</a>
	...
	<div class="card-body table-responsive">
		<table id="IliTable" class="table table-sm table-striped table-borderless table-condensed table-hover ">
...
<form method="post">
<div class="modal" id="form_ili" tabindex="-1" role="dialog">
  <div class="modal-dialog modal-dialog-centered modal-xl" role="dialog">
   <div class="modal-content">
   	<div class="modal-header">
	<h2 class="modal-title text-success"><i class="fa fa-file-pdf-o"></i> Influenza-like Illness (ICD 10 Code: J11)</h2>
	<button class="btn btn-danger btn-ladda  float-right" data-style="zoom-in" ria-label="Close" data-dismiss="modal">
				<i class="fa fa-close"></i> 
				</button>
	</div>
	  <div class="modal-body">
		<embed id="printilipdfmodal" type="application/pdf" frameborder="2" width="100%" height="800px">
      </div>
      <div class="modal-footer bg-secondary">
      </div>
    </div>
	...
	<input name="hpercode" id ="hpercode" type="hidden">
 	<script src="<?php echo base_url()?>assets/scripts/pidsr_ili/delete.js"></script> 
	...
	function pdfprint(){
		...
		$('#printpdf').click('click',function(){
			$('#form_ili').modal({ backdrop: 'static' }).draggable();
			var x = document.getElementById("printilipdfmodal");
			x.src = baseURL+"PIDSR_ILI/Ili_pdf";
			/*x.src = baseURL+"PIDSR_ILI/Ili_pdf/"+from+'/'+to;*/
			});;
	}
	function PatientLogList()
	{
		var data = new Object();
				data.id = "IliTable";
				data.link = baseURL+"PIDSR_ILI/iliList";
				/* data.link = baseURL+"PIDSR_ILI/iliList/"+from+"/"+to;*/
				data.type = "POST";
				data.coldef =[
				{
				targets: [0,1],
	...
	$("#IliTable").on("click",".ModalDeleteIli",function(){
		var data = $(this).data();
		$('#DeleteIli').modal({ backdrop: 'static' }).draggable();
		$("#formIdentification").val('delete');
		$("#deletecode").val(data['enccode']);

	});

	$('#IliTable').on('click','.ModalPatientInfo',function(){
	
		$('#ModalPatientInfo').modal({ backdrop: 'static' }).draggable();
