...
<div class="card card-accent-success">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table id="ILTable" class="table table-sm table-bordered table-condensed table-hover" width="100%">
                <thead>
                    <tr>
...
<form id="frmDeniedListPDF">
    <div class="modal fade" id="ModalDeniedList" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-vertical dialog modal-xl" role="dialog">
            <div class="modal-content card-accent-success">
                <div class="modal-header">
                    <h4 class="modal-title text-success"><i class="fa fa-file-pdf-o"></i> Denied</h4>
                    <button class="btn btn-danger btn-square float-right" data-dismiss="modal" data-style="zoom-in"><i class="fa fa-close"></i></button>
                </div>
                <div class="modal-body">
                    <iframe id="DeniedListPDF" type="application/pdf" width="100%" height="800px"></iframe>
                </div>
...
<script type="text/javascript">
    ...
    function DL(from, to, filters) {
        var data = new Object();
        data.id = "ILTable";
        data.link = baseURL + "Rep_eClaims/DeniedLists/" + from + "/" + to + "/" + filters;
        data.type = "POST";
        data.coldef = [{
    ...
    $('#printtl').on('click', function(data) {
        $('#ModalDeniedList').modal('show');

        var from = $.session.get('from');
        var to = $.session.get('to');
        var filters = $.session.get('filters');
        var src = baseURL + 'Rep_eClaims/DeniedList_pdf/' + from + "/" + to + "/" + filters;
        var x = document.getElementById("DeniedListPDF").src = src;

        $("#DeniedListPDF").attr("src", src);
    });

    $('#ModalDeniedList').on('hidden.coreui.modal', function() {
        $("#DeniedListPDF").removeAttr("src").attr("src", "");
    });
