...
<div class="card card-accent-success">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table id="WCLTable" class="table table-sm table-bordered table-hover" width="100%">
                <thead>
                    <tr>
                        <th class="align-middle text-center">Receipt Ticket Number</th>
                        ...
                        <th class="align-middle text-center">Voucher Number</th>
                        <th class="align-middle text-center">Voucher Date</th>
                        <th class="align-middle text-center">Check No</th>
                        <th class="align-middle text-center">Check Date</th>
                        <th class="align-middle text-center">Check Amount</th>
                        <th class="align-middle text-center">Claim Amount</th>
                        <th class="align-middle text-center"></th>
                    </tr>
                </thead>
            </table>
...
<form id="frmWithChequeListPDF">
    <div class="modal fade" id="ModalWithChequeList" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-vertical dialog modal-xl" role="dialog">
            <div class="modal-content card-accent-success">
                <div class="modal-header">
                    <h4 class="modal-title text-success"><i class="fa fa-file-pdf-o"></i> With Cheque</h4>
                    <button class="btn btn-danger btn-square float-right" data-dismiss="modal" data-style="zoom-in"><i class="fa fa-close"></i></button>
                </div>
                <div class="modal-body">
                    <iframe id="WithChequeListPDF" type="application/pdf" width="100%" height="800px"></iframe>
                </div>
...
<script type="text/javascript">
    ...
    function WCL(from, to, filters) {
        var data = new Object();
        data.id = "WCLTable";
        data.link = baseURL + "Rep_eClaims/WithChequeLists/" + from + "/" + to + "/" + filters;
        data.type = "POST";
        data.coldef = [{
            ...
            {
                targets: [0, 1, 2, 3, 5, 6, 10, 11, 12, 13],
    ...
    $('#printtl').on('click', function(data) {
        $('#ModalWithChequeList').modal('show');

        var from = $.session.get('from');
        var to = $.session.get('to');
        var filters = $.session.get('filters');
        var src = baseURL + 'Rep_eClaims/WithChequeList_pdf/' + from + "/" + to + "/" + filters;
        var x = document.getElementById("WithChequeListPDF").src = src;

        $("#WithChequeListPDF").attr("src", src);
    });

    $('#ModalWithChequeList').on('hidden.coreui.modal', function() {
        $("#WithChequeListPDF").removeAttr("src").attr("src", "");
    });
