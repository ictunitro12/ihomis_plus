...
<form id="frmReturnListPDF">
    <div class="modal fade" id="ModalReturnList" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-vertical dialog modal-xl" role="dialog">
            <div class="modal-content card-accent-success">
                <div class="modal-header">
                    <h4 class="modal-title text-success"><i class="fa fa-file-pdf-o"></i> Returned</h4>
                    <button class="btn btn-danger btn-square float-right" data-dismiss="modal" data-style="zoom-in"><i class="fa fa-close"></i></button>
                </div>
                <div class="modal-body">
                    <iframe id="ReturnListPDF" type="application/pdf" width="100%" height="800px"></iframe>
                </div>
...
<script type="text/javascript">
    ...
    function RL(from, to, filters) {
        var data = new Object();
        data.id = "ILTable";
        data.link = baseURL + "Rep_eClaims/ReturnLists/" + from + "/" + to + "/" + filters;
        data.type = "POST";
        data.coldef = [{
    ...
    $('#printtl').on('click', function(data) {
        $('#ModalReturnList').modal('show');

        var from = $.session.get('from');
        var to = $.session.get('to');
        var filters = $.session.get('filters');
        var src = baseURL + 'Rep_eClaims/ReturnList_pdf/' + from + "/" + to + "/" + filters;
        var x = document.getElementById("ReturnListPDF").src = src;

        $("#ReturnListPDF").attr("src", src);
    });

    $('#ModalReturnList').on('hidden.coreui.modal', function() {
        $("#ReturnListPDF").removeAttr("src").attr("src", "");
    });
