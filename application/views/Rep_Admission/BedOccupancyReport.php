...
<div class="card card-accent-success">
	<div class="card-header">
		<div class="row">
				...
				<div class="col-md-6">
					...
					<button type ="button" id="printBedOccpdf" title="Print PDF"  class="btn btn-success pull-right btn-md" data-style="zoom-in"><i class="fa fa-file-pdf-o"></i>
						</button>
			   	 	</div>
			   	</div>
		</div>
	...
	<div class="card-body table-responsive">
		<table id="BedOccupancyReports" class="table table-sm table-striped table-borderless table-condensed table-hover ">
			<thead class="thead-dark">
				...
				<th>WARD</th>
			    <th>HEALTH REC.#</th>
				<th>PATIENT'S NAME (<small>Lastname, Firstname Middlename Suffix</small>)</th>
				<th>ROOM MOVE</th>
...
<div class="modal" id="form_patrec" tabindex="-1" role="dialog">
  <div class="modal-dialog modal-dialog-centered modal-xl" role="dialog">
   <div class="modal-content">
	...
	function bedOccList(from,to)
	{
		$.ajax({
		url:baseURL + "Rep_Admission/bedOccRep/"+from+'/'+to,
		type:"POST",
		data: "JSON",
		dataType: "JSON",
		processData:false,
		...
		success: function(data,response){
			...
			$('#printBedOccpdf').click('click',function(){
			$('#form_patrec').modal({backdrop:'static'}).draggable();
			var x = document.getElementById("printadmittedpdfmodal").src = baseURL+"Rep_Admission_pdf/BdOccupancyRep/"+from+"/"+to;
			});
			var table = $("#BedOccupancyReports").dataTable({
			data: data,
			destroy: true,
