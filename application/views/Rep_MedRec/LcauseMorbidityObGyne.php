...
$(document).ready(function()
   ...
   $('#DateSearch').modal({backdrop:'static'});
	...
	$('#filter').on('click',function(){
		...
		{
	    window.open(baseURL+"Rep_MedicalRecords/LeadCausesMorbidityObGyne/monthly/"+$("#mnth").val(),'name'); 
		}
		else if($("#quarterly").prop('checked') == true)
		{
		var year = $("#qyear").val();
	    window.open(baseURL+"Rep_MedicalRecords/LeadCausesMorbidityObGyne/quarterly/"+$("#squarter").val()+year); 
		}else if($("#annual").prop('checked') == true){
			var year = $("#qyear").val();
			window.open(baseURL+"Rep_MedicalRecords/LeadCausesMorbidityObGyne/yearly/"+year); 
		}
	});	
	</script>		
