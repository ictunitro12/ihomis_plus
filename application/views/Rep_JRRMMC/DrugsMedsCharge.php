...
<form id="dateRange" method="post">
			...
			<table id="TLTable" class="table table-sm table-striped table-bordered table-condensed table-hover" width="100%">
				<thead class="thead-dark">
					<tr>
						...
						<th class="align-middle text-center" rowspan="2">Date Charged<br> to Patient</th>
	...
	<form id="frmDrugsMedsChargePDF">
		<div class="modal fade" id="ModalDrugsMedsCharge" tabindex="-1" role="dialog">
			<div class="modal-dialog modal-vertical dialog modal-xl" role="dialog">
				<div class="modal-content card-accent-success">
					<div class="modal-header">
						<h4 class="modal-title text-success"><i class="fa fa-file-pdf-o"></i>&nbsp; Report of Drugs and Medicines Charged to Patients </h4>
						<button class="btn btn-danger btn-square float-right" data-dismiss="modal" data-style="zoom-in"><i class="fa fa-close"></i></button>
					</div>
					<div class="modal-body">
						<iframe id="DrugsMedsChargePDF" type="application/pdf" width="100%" height="800px"></iframe>
					</div>
	...
	<script type="text/javascript">
		...
		function TL(from, to) {
			var data = new Object();
			data.id = "TLTable";
			data.link = baseURL + "Rep_JRRMMC/DrugsMedsChargeList/" + from + "/" + to;
			data.type = "POST";
			data.coldef = [{
		...
		$('#printtl').on('click', function(data) {
			$('#ModalDrugsMedsCharge').modal('show');

			var from = $.session.get('from');
			var to = $.session.get('to');
			var src = baseURL + 'Rep_JRRMMC/DrugsMedsCharge_pdf/' + from + "/" + to;
			var x = document.getElementById("DrugsMedsChargePDF").src = src;

			$("#DrugsMedsChargePDF").attr("src", src);
		});

		$('#ModalDrugsMedsCharge').on('hidden.coreui.modal', function() {
			$("#DrugsMedsChargePDF").removeAttr("src").attr("src", "");
		});
