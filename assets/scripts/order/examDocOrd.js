  ...
  data.coldef = [
    ...
    },
    {
      ...
      render: function (data, type, row) {
        switch (data) {
          case "STAT":
            return '<div class="bg-danger text-white p-2">URGENT</span>';
            break;
          case "ROUTIN":
            return '<div class="bg-warning text-white p-2">ROUTINE</span>';
            break;
          case "UNKNO":
            return '<div class="bg-primary text-white p-2">UNKNOWN</span>';
            break;
          default:
            return '<div class="bg-info text-white p-2">NO PRIORITY</span>';
        }
      },
    },
  ];
...
$("#docOrdexam").on("click", "#btnEditExam_edit", function () {
  ...
  var proccode = encodeURIComponent(
    encodeURIComponent($(this).data("proccode"))
  );
  $.ajax({
    type: "POST",
    ...
    success: function (data) {
      ...
      $("#rfee3").val(obj["rfee3"]);
      if (obj["dopriority"] !== "") {
        $("#priority option[value='" + obj["dopriority"] + "']").prop(
          "selected",
          true
        );
      }
...
function selExamlist(id = 0) {
  ...
  (data.columns = [
    ...
    },
    {
      ...
      render: function (data, type, full, meta) {
        return (
          '<input type="text" class="form-control form-control-sm"  name="procrate[]"  value="' +
          data +
          '">'
        );
      },
    },
    {
      ...
      render: function (data, type, full, meta) {
        return (
          '<input type="text" class="form-control form-control-sm"  name="remarks[]" placeholder="Remarks" value="' +
          data +
          '">'
        );
      },
...
$("#frmProc").submit("click", function (event) {
  ...
  for (var i = 0; i < rowData.length; i++) {
    var dataString =
      ...
      rowData[i]["remarks"] +
      "&prio=" +
      $($("#examList").DataTable().cell(i, 5).node()).find("select").val() +
      "&notes=" +
      $($("#examList").DataTable().cell(i, 8).node()).find("input").val() +
      "&reason=" +
      $($("#examList").DataTable().cell(i, 9).node()).find("input").val() +
      "&curcode=" +
      rowData[i]["curcode"] +
      "&prikey=" +
      rowData[i]["prikey"] +
