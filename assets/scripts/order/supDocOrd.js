...
$("#btnSelectSupp").on("click",function(){
	...
	$.each(rowData, function (i, d) {
		...
		'<input type="hidden" class="form-control form-control-sm" readonly value='+i+' name="row[]" id="row[]">'
		]).draw(true);
	});
					...
					switch(data) 
						{
							...
							default : return '<div class="p-1 bg-danger text-white">UNKNOWN</span>'; break;
						}
					},
				},
				{
					...
					switch(data) 
						{
							...
							case 'NEWOR' : return '<div class="p-1 bg-warning text-white">New</span>'; break;
	...
	$("#frmSupOrder").on('submit',function(){
		...
		var orderListArray = [];
			...
			{     
				...
				var cl2comb=$(orderSuppList.cell(i, 1).node()).find('input[name=cl2comb]').val();
				var qty=$(orderSuppList.cell(i, 3).node()).find('input[name=qty]').val();
				var subprio=$(orderSuppList.cell(i, 4).node()).find('select[name=subprio]').val();
				var remarks=$(orderSuppList.cell(i, 4).node()).find('input[name=remarks]').val();
...
return false;
	...
	var cl2comb=encodeURIComponent(encodeURIComponent($(this).data('cl2comb')));
			...
			async:false,
				success: function(data)
				{
					...
					$("#prioEdit option[value=" + obj["dopriority"] + "]").prop("selected",true);
 ...
 function setSubCategory(code)
 {
	...
	$("#docOrdsupp").on('click','tr',function(){
	 ...
	 $("#btnCancelSupp").on('click',function(){
		...
		$.each(rowData, function (index, data) {
			$('#listDocoSupp').prepend('<li class="text-danger list-group-item">'+'*'+data[4] + 
	...
	$("#cancelSupplyModal").on('hidden.coreui.modal',function(){
		...
		$("#docOrdsupp").DataTable().$('tr.selected').removeClass('bg-success text-white selected');
