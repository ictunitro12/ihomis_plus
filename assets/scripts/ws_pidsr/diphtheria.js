...
function Ws_diphtheria_List() {
	var table = $("#ws_DiphtheriaTbl").DataTable({
		processing: true,
		searchable: true,
		destroy: true,
		pageLength: 25,
		...
		ajax: {
			url: baseURL + "PIDSR_Webservice/Diphtheria_List",
			type: "POST",
		},

	});
	$('#uploaddata').on('click', function (e) {
		const form = new FormData(document.getElementById("diphtheria_form"));
			...
			if (width >= 100) {
				...
				$.ajax({
					...
					success: function (data, response, type) {
						...
						/* var obj = $.parseJSON(data.webresponse);
						var objdata = $.parseJSON(data.data); */
						const myJSON = data.webresponse[0];
						const myArray = JSON.parse(myJSON);
						console.log(myArray);
						/* if (obj.statusCode[0] == '201') {
							toastr.success('<b>' + objdata.diseasedata[0].firstname + ' ' + objdata.diseasedata[0].middlename + ' ' + objdata.diseasedata[0].lastname + ' ' + objdata.diseasedata[0].suffixname + '</b><br>' + obj.messages[0], 'SUCCESS', { timeout: 10000 });
						} */
						$("#ModalProgressBar2").modal('hide');
						window.setTimeout(function () { $('#checkAll_patient').prop('checked', false); }, 1000);
						/* window.setTimeout(function () { $("#ws_DiphtheriaTbl").DataTable().ajax.reload(); }, 1000); */
					},
...
/* for (var a = 0; a < data.webresponse.length; a++) {
							...
							if (obj.statusCode == '401') {
								...
								for (var b = 0; b < myArray.length; b++) {
									...
									$.ajax({
											url: baseURL + "PIDSR_Webservice/update_diphtheria/" + hpercode,
											type: "POST",
										});
										window.setTimeout(function () { $("#ws_DiphtheriaTbl").DataTable().ajax.reload(); }, 1000); 
									}
