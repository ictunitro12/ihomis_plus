...
function Ws_ili_List() {
	var table = $("#ws_IliTbl").DataTable({
		processing: true,
		searchable: true,
		destroy: true,
		pageLength: 25,
		...
		ajax: {
			url: baseURL + "PIDSR_Webservice/Ili_list",
			type: "POST",
		},

	});
	$('#uploaddata').on('click', function (e) {
		const form = new FormData(document.getElementById("ili_form"));
							...
							if (obj.statusCode == '401') {
								...
								for (var b = 0; b < myArray.length; b++) {
									if (myArray[0].statusCode == '201' && myArray[1].statusCode == '201' && myArray[2].statusCode == '201') {
										...
										$.ajax({
											url: baseURL + "PIDSR_Webservice/update_ili/" + hpercode,
											type: "POST",
										});
										window.setTimeout(function () { $("#ws_IliTbl").DataTable().ajax.reload(); }, 1000);
									}
