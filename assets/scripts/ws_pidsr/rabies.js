...
function Ws_rabies_List() {
	var table = $("#ws_RabiesTbl").DataTable({
		processing: true,
		searchable: true,
		destroy: true,
		pageLength: 25,
		...
		},
		{
			render: function (data) {
				var date_onset = new Date(data['date_on_set']);
		...
		ajax: {
			url: baseURL + "PIDSR_Webservice/Rabies_list",
			type: "POST",
		},

	});
	$('#uploaddata').on('click', function (e) {
		const form = new FormData(document.getElementById("rabies_form"));
							...
							if (obj.statusCode == '401') {
								...
								for (var b = 0; b < myArray.length; b++) {
									if (myArray[0].statusCode == '201' && myArray[1].statusCode == '201' && myArray[2].statusCode == '201') {
										...
										$.ajax({
											url: baseURL + "PIDSR_Webservice/update_rabies/" + hpercode,
											type: "POST",
										});
										window.setTimeout(function () { $("#ws_RabiesTbl").DataTable().ajax.reload(); }, 1000);
									}
