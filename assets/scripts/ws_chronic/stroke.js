...
function Ws_stroke_List() {
	var table = $("#ws_StrokeTbl").DataTable({
		processing: false,
		searchable: true,
		destroy: true,
		pageLength: 25,
		...
		ajax: {
			url: baseURL + "CHRONIC_Webservice/Stroke_list",
			type: "POST",
		},
	...
	$('#uploaddata').on('click', function (e) {
		...
		const form = new FormData(document.getElementById("stroke_form"));
	...
	if (i == 0) {
		...
		function frame() {
			if (width >= 100) {
				clearInterval(id);
				$.ajax({
					url: baseURL + "CHRONIC_Webservice/push_Stroke",
					type: "POST",
					data: form,
					dataType: 'JSON',
					processData: false,
					...
					success: function (data, response, type) {
						...
						for (var l = 0; l < data.code.length; l++) {
							if (data.code[l].code == '104') {
								...
								$.ajax({
									url: baseURL + "CHRONIC_Webservice/update_stroke_registryno/" + h_patient_id + '/' + registry_no,
									type: "POST",
								});
							}
							else {
						...
						window.setTimeout(function () { $("#ws_StrokeTbl").DataTable().ajax.reload(); }, 1000);
					},
