	...
	function order(enccode)
	{
		...
		"columns" : [
			...
			{ 
				...
				render: function(data, type, row)
				{
					return  (data > 0 ? '<div class="text-primary">YES</div>' : (data ==  0 ? '<div class="text-danger">NO</div>' : '<div class="text-secondary">NONE</div>'));
				}	
			},
			{ 
					data:"intkey",
		...
		orderTable.on('click','tr',function(){
			var rowData = orderTable.row(this).data();
		...
		$('#modalFilm').on('hidden.coreui.modal',function(){
			$('#modalResult').modal('show');
		});

		$('#modalfilmEntry').on('hidden.coreui.modal',function(){
			$('#modalResult').modal('show');
		});
