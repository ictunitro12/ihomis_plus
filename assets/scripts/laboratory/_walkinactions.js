...
$("#btnCharge_add").on("click",function(){
		$('div[id$=_acct]').load(baseURL+"Billing/viewwlkin",function(){
					$.get(baseURL+"Laboratory/genChargeSlipw/"+null, function (data) {$("#chrgslip").val($.trim(data));});
					$("#costcntr option[value="+'LABOR'+"]").prop('selected', true);
...
/* 		
	...
	$("#walkinTbl").on('click','#btnEditWalkin',function(){
		var orno = atob($(this).data('code'));
			...
			$("#paid_amt").prop('readonly',true);
			$("#costcntr option[value=" +'LABOR'+ "]").prop('selected', true);
		$.get(baseURL+"Laboratory/walkinLaborInfo/"+orno, function (data){
		...
		$.get(baseURL+"Laboratory/walkinList/"+orno, function (data){
			...
			$('#chargesList tbody').on('click','#btnremove',function(){
						...
						$.ajax({
						type:'POST',
						url: baseURL+"Laboratory/removeitem",
						type:'POST',
						data:chargeDtl,
						dataType:'JSON',
	...
	$('#frmDeleteWalkin').on('submit', function(e){
      var form = this;
		$.ajax({
		type : "POST",
		url  : baseURL+"Laboratory/deleteWalkPharma",
	...
	if(row.length > 0 ){
		...
		$.ajax({
				...
				data:header,
				dataType:'JSON',
				url: baseURL+"Laboratory/saveWalkin",
				success: function(data,response){
	  ...
	  for(var i = 0; i <row.length; i++)
	  {
		...
		var intkey=chrgslip.concat(itemcode,row[i]); 
			...
			$.ajax({
				type:'POST',
				url: baseURL+"Laboratory/saveWalkinDtl",
				type:'POST',
				data:chargeDtl,
				dataType:'JSON',
				success: function(data,response){
		 ...
		 rules:{
			...
			orno:{
					...
					remote:{	
					  ...
					  param:{
						url: baseURL+"Laboratory/checkOrno",	
						type:'POST'
					  }
					},
				},
