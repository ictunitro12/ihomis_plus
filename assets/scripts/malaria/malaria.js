	...
	function TravelBList(enccode, hpercode)
	{
		var hpercode= encodeURIComponent(encodeURIComponent(hpercode));
		var enccode= encodeURIComponent(encodeURIComponent(enccode));

		var data = new Object();
		data.id = "TravelBTable";
		data.link = baseURL+"Malaria/TravelBList/" + enccode + "/" +hpercode;
		data.type = "POST";
		data.coldef=[
		{
			targets : [0,1,2],
	...
	function ConTraceList(enccode, hpercode)
	{
		var hpercode= encodeURIComponent(encodeURIComponent(hpercode));
		var enccode= encodeURIComponent(encodeURIComponent(enccode));

		var data = new Object();
		data.id = "ConTraceTable";
		data.link = baseURL+"Malaria/ConTraceList/" + enccode + "/" +hpercode;
		data.type = "POST";
		data.coldef=[
		{
			targets : [0,1,2],
	...
	function getPatientData(enccode,hpercode)
	{
		...
		$.ajax({
			url: baseURL + "Malaria/getPatientData/"+enccode+ "/" + hpercode,
			type:"POST",
			data:"JSON",
			processData:false,
			cache:false,
			...
			success: function(data,response){
				...
				$('#info_patlast').val(obj['patlast']);
				$('#info_patfirst').val(obj['patfirst']);
				$('#info_patmid').val(obj['patmiddle']);
				...
				$("#copd_dateconsult").val(consults);
				$("#dateconsult").val(consults);

				var encounterdte = obj['encounter_date'];
				var encounterdate = ((encounterdte == "1970-01-01 00:00:00") || (encounterdte == null)) ? "0000-00-00 00:00:00" : setTimeLocale(encounterdte);
				$("#copd_datediag").val(encounterdate);
				$("#datediag").val(encounterdate);


				var dob = obj['patbdate'];
				var dobs = ((dob == "1970-01-01 00:00:00") || (dob == null)) ? "0000-00-00 00:00:00" : setTimeLocale(dob);
				$("#info_dob").val(dobs);			
	...
	function checkLabMalaria(enccode,hpercode)
	{
		enccode= encodeURIComponent(encodeURIComponent(enccode));
		hpercode= encodeURIComponent(encodeURIComponent(hpercode));
		...
		$.ajax({
			type: "POST",
			url: "../Malaria/checkAssessmentExist/"+enccode+ "/" + hpercode,
			data: "JSON",
			success: function(data)
			{ 
				if(data==1)
				{	
					toastr.warning('Patient already exist!<br>You may edit MSWD Form in.. </br> <button type="button" class="btn btn-info   btn-block btn-md btn-toastr" onclick="location.reload();">Cancel</button>' ,'Warning');		
				}
				else
				{ 
					window.location.replace(baseURL+"Malaria/MalariaAssessment" );
				}
			},
			error: function(data)
			{
	...
	function checkPatient(hpercode)
	{
		...
		$.ajax({
			type: "POST",
			url: baseURL + "Malaria/checkPatient/"+ hpercode,
			data: "JSON",
			success: function(data)
			{ 
				if(data==1)
	...
	function getTypeEncounter(enccode, hpercode)
	{
		...
		$.ajax({
			url: baseURL + "Malaria/getTypeEncounter/"+enccode + "/" + hpercode,
			type:"POST",
			data:"JSON",
			processData:false,
			cache:false,
	...
	*/
	function initMalariawizard(enccode, hpercode)
	{
		Malariawizard();
		_validateWizard();
	...
	function PatientAssessmentFormData(enccode,hpercode) {
		...
		$.ajax({
			type: "POST",
			url  : baseURL+"Malaria/updateAssessmentForm/" + enccode + "/"+ hpercode,
			data: {
				'enccode' : enccode,
				'hpercode': hpercode
			},
