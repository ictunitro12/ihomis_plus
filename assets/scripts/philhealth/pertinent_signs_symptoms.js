...
$(document).ready(function () {
  ...
  $("#pertinent_signs_symptoms_form").validate({
    rules: {
      pain_site: {
        required: {
...
function validate_pertinent_signs_symptoms_form() {
  if ($("#pertinent_signs_symptoms_form").valid() == true) {
    return true;
  } else {
...
$("#pertinent_signs_symptoms_form").submit(function (e) {
  e.preventDefault();
  ...
  $.each(
    $("#pertinent_signs_symptoms_form input[type=checkbox]").filter(function (
      idx
    ) {
...
submitPertinentSignsSymptoms = function (formData, enccode, loadData = false) {
  $.ajax({
    url: baseURI + "pertinent-signs-symptoms/" + enccode + "/save/",
    type: "POST",
    data: formData,
    dataType: "JSON",
    processData: false,
...
getPertinentSignsSymptomsData = function (enccode) {
  $.ajax({
    url: baseURI + "pertinent-signs-symptoms/" + enccode + "/get/",
    type: "GET",
    dataType: "JSON",
    processData: false,
    contentType: false,
...
bindPertinentSignsSymptomsData = function (data) {
  if (data.abdominal_cramp_pain !== null && data.abdominal_cramp_pain !== "") {
