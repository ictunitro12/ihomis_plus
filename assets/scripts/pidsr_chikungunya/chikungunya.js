...
function initChikungunya(hpercode, enccode) {
	CivilStatus();
	// uppercase();
	SelIndig();
...
function initChikungunyaEdit(hpercode, enccode) {
	initcheck();
	CivilStatus();
	// uppercase();
	getChikungunyaData(enccode, hpercode);
	$('#enccode').val(enccode);
	$('#formIden').val('update');
...
function getPatientData(enccode) {
	...
	$.ajax({
		url: baseURL + "PIDSR_Chikungunya/getPatientData/" + enctr,
		type: "POST",
		data: "JSON",
		processData: false,
		cache: false,
