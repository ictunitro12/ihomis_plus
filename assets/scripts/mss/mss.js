	...
	function checkPatient(hpercode)
	{
	  ...
	  $.ajax({
	  type: "POST",
	  url: baseURL + "MSS/checkPatient/"+ hpercode,
	  data: "JSON",
	...
	function initMsswizard(enccode, hpercode)
	{
	...
	function getPatientData(enccode, hpercode)
	{
		...
		hpercode= encodeURIComponent(encodeURIComponent(hpercode));
			...
			$.ajax({
			url: baseURL + "MSS/getPatientData/"+enccode + "/" + hpercode,
			type:"POST",
			data:"JSON",
			processData:false,
			cache:false,
	...
	function MSSwizard()
	{
		...
		.on('click',function(){$('#AssessmentSummary').modal('show');AssessmentSummary();});
...
function AssessmentSummary()
	{ 	
		...
		var hpercode= $('#hpercode_assess').val();
		var enccode=$('#enccode_assess').val();
		$('#saveenccode').val(enccode);
		$('#savehpercode').val(hpercode);
