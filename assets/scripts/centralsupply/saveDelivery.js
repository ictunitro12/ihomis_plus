...
$('#formDeliveries').on('submit', function(e) {
    if ($('#formDeliveries').valid()) 
	{
        ...
        for(let i = 0; i< rows.length; i++){
            ...
            var cl1comb= $(table.cells(rows[i], 5).nodes()).text();
            var cl2comb= $(table.cells(rows[i], 6).nodes()).text();
            var cl2code= $(table.cells(rows[i], 7).nodes()).text();
            var formcode= $(table.cells(rows[i], 8).nodes()).text();


            var unitprice= $(table.cells(rows[i], 9).nodes()).find('input[name=unitprice]').val();
            var expiry= $(table.cells(rows[i], 10).nodes()).find('input[name=expire]').val();
            var remarks= $(table.cells(rows[i], 11).nodes()).find('input[name=remarks]').val();
            ...
            var expiry_elem = $(table.cell(rows[i], 10).node())
            .find("input")
            .get(0);
            ...
            dataArr.cl1comb= cl1comb;
            dataArr.cl2comb= cl2comb;
            dataArr.cl2code= cl2code;
        ...
        $.ajax({
			...
			dataType:'JSON',
			url: baseURL+"Central_Supply/saveDelivery",
