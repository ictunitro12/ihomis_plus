...
/* 		
	...
	$("#walkinTbl").on('click','#btnEditWalkin',function(){
		var orno = atob($(this).data('code'));
			...
			$("#paid_amt").prop('readonly',true);
			$("#costcntr option[value=" +'PHARM'+ "]").prop('selected', true);
		$.get(baseURL+"Pharmacy/walkinPharmaInfo/"+orno, function (data){
		...
		$.get(baseURL+"Pharmacy/walkinPharmaList/"+orno, function (data){
			...
			$('#chargesList tbody').on('click','#btnremove',function(){
						var rowIndex=chargesPatwalk.row($(this).parents('tr')).index();
							...
							var itemcode = $(chargesPatwalk.cell(rowIndex, 8).node()).find('input').val();
							var lot = $(chargesPatwalk.cell(rowIndex, 9).node()).find('input').val();
							var ctr = $(chargesPatwalk.cell(rowIndex, 10).node()).find('input').val();
								console.log(ctr);
						var chargeDtl =
								'&chrgcod='+chrgcod+
								'&dmdcomb='+itemcode+
						...
						$.ajax({
						type:'POST',
						url: baseURL+"Pharmacy/removeitemPharma",
						type:'POST',
						data:chargeDtl,
						dataType:'JSON',
	...
	$('#frmDeleteWalkin').on('submit', function(e){
      var form = this;
		$.ajax({
		type : "POST",
		url  : baseURL+"Pharmacy/deleteWalkPharma",
	...
	if(row.length > 0 ){
		...
		$.ajax({
				...
				data:header,
				dataType:'JSON',
				url: baseURL+"Pharmacy/saveWalkinPHARMA",
				success: function(data,response){
	  ...
	  for(var i = 0; i <row.length; i++)
	  {
		...
		var intkey=chrgslip.concat(itemcode,ctr,row[i]); 
			var chargeDtl = 'intkey='+intkey+
					'&orno='+orno+
					'&formType='+formType+
					'&itemcode='+itemcode.concat(ctr)+
					'&proccode='+itemcode.concat(ctr)+
					'&chrgcod='+chrgcod+
					'&dmdcomb='+itemcode+
					'&dmdctr='+ctr+
					'&qty='+qty+
			...
			$.ajax({
				type:'POST',
				url: baseURL+"Pharmacy/saveWalkinDtlPharm",
				type:'POST',
				data:chargeDtl,
				dataType:'JSON',
				success: function(data,response){
		 ...
		 rules:{
			...
			orno:{
					...
					remote:{	
					  ...
					  param:{
						url: baseURL+"Pharmacy/checkOrno",	
						type:'POST'
					  }
					},
				},
