function generatechargeslip(){
    ...
    for(let i = 0; i< rows.length; i++){
      ...
      var issuedte= $(table.cells(rows[i], 1).nodes()).find('input').val();
        ...
        var issue_elem = $(table.cell(rows[i], 7).node())
          .find("input")
          .get(0);
       
        var lotno_elem = $(table.cell(rows[i], 3).node())
          .find("input")
          .get(0);
          if (issue == 0) {
            $('#listOrder li').remove();
            table.rows('.selectedOrder').deselect();
            $(issue_elem).next("#issueQtyError").remove();
            $(issue_elem).removeClass("is-valid");
            $(issue_elem).addClass("is-invalid");
            $(issue_elem).after(
              '<div class="invalid-feedback" id="issueQtyError">Qty is required!</div>'
            );
            ...
            $(issue_elem).next("#issueQtyError").remove();
            $(issue_elem).removeClass("is-invalid");
            $(issue_elem).addClass("is-valid");
            $(issue_elem).after(
              '<div class="invalid-feedback" id="issueQtyError">Qty is required!</div>'
            );
          }
        ...
        if (lotno == 0) {
            $('#listOrder li').remove();
            table.rows('.selectedOrder').deselect();
          $(lotno_elem).next("#lotnoError").remove();
          $(lotno_elem).removeClass("is-valid");
          $(lotno_elem).addClass("is-invalid");
          $(lotno_elem).after(
            '<div class="invalid-feedback" id="lotnoError">Lotno is required!</div>'
          );
        
          hasError = true;
        } else {
         $(lotno_elem).next("#lotnoError").remove();
         $(lotno_elem).removeClass("is-invalid");
         $(lotno_elem).addClass("is-valid");
         $(lotno_elem).after(
           '<div class="invalid-feedback" id="lotnoError">Lotno is required!</div>'
         );
        ...
        if(lotno!=0 && issue!=0){
        $.get(baseURL+"Pharmacy/genChargeSlip", function(data, status){
            $('#chargeslip').text(data);
            $("#drugOrderModal").modal('show');
         });
      }else{
            $('#listOrder li').remove();
      }
...
$("#drugOrderModal").on('hidden.coreui.modal',function(){
    $('#listOrder li').remove();
 });