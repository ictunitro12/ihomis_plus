...
function dmStock() {
	...
	data.dataType = 'JSON';
	data.data =  {typeAccradio :$('.typeofaccount,input[name=typePharm]:checked', '#typeofAccount').val()};
	data.type = "POST";
	data.search = true;
	data.destroy = true;
	data.paging = true;
			...
			},
			{
				...
				render: function (data, type, row) {
					return "<div class=btn-group role=group>" +
						...
						"<a class='dropdown-item'  data-dmdcomb='" + row.dmdcomb + "' data-dmdctr='" + row.dmdctr + "' data-dmsub='" + row.dmhdrsub + "'  data-formcode='" + row.formcode + "'  data-baldteasof='" + row.baldteasof + "' id='btnTypeAcc' href='#'><i class='fa fa-calculator'></i> &nbsp Account</a>" +
...
var chart = Highcharts.chart('viewChart', 
   {
	...
	$('.typeofaccount').on('change',function(){
		$.ajax({
			...
			data: { typeAccradio:$('input[name=typePharm]:checked', '#typeofAccount').val()},
...
function druginfo(dmdcomb, dmdctr) {
	$.ajax({
		...
		success: function (data, response) {
			...
			$("#dmhdrsub").val(obj['dmhdrsub']);drugPicLoc
			var drugPic = $.md5(obj['dmdcomb'].concat(obj['dmdctr'],obj['baldteasof']));
			...
			http.open('HEAD', urlPic, false);
                http.send();
                if (http.status === 200) {
					$("#drugPhoto").attr("src",urlPic+"?timestamp=" + new Date().getTime());
                } else {
	...
	data.columns = [
		...
		{ data: "dmdrem" },
		{
			...
			if(row.dmselprice==null){
				...
				"</button>" +
				"<div class='dropdown-menu' aria-labelledby='btnGroupDelivDrop'>" +
				"<a  class='dropdown-item' data-lotno='" + row.lotno + "' data-dmdcomb='" + row.dmdcomb + "' data-dmdsub='" + row.dmhdrsub + "' data-dmdctr='" + row.dmdctr + "' data-dmdprdte='" + row.dmdprdte + "' id='btnEditAcc' href='#'>&nbsp Edit</a>" +
				"</div>" +
			"</div>";
				...
				"<div class='dropdown-menu' aria-labelledby='btnGroupDelivDrop'>" +
				"<a  class='dropdown-item' data-lotno='" + row.lotno + "' data-dmdcomb='" + row.dmdcomb + "' data-dmdsub='" + row.dmhdrsub + "' data-dmdctr='" + row.dmdctr + "' data-dmdprdte='" + row.dmdprdte + "' id='btnEditAcc' href='#'>&nbsp Edit</a>" +
				"<a	 class='dropdown-item'  data-serveno='" + row.serve_price + "' data-serveprice='" + row.serve_price + "'  data-deliv='" + row.delintkey + "' data-lotno='" + row.lotno + "' data-dmdcomb='" + row.dmdcomb + "' data-dmdsub='" + row.dmhdrsub + "' data-dmdctr='" + row.dmdctr + "' data-dmdprdte='" + row.dmdprdte + "'  data-stockbal='" + row.stockbal + "'   data-begbal='" + row.begbal + "' data-price='" + row.dmselprice + "'   data-form='" + row.unitcode + "'   data-uprice='" + row.dmduprice + "'  data-expiry='" + row.expiry + "'  id='btnNewPrice' href='#'>&nbsp New Price</a>" +
				"</div>" +
			"</div>";
...
$("#btnAddPhoto").on('click',function(){
	...
	var drugPic = $.md5($("#dmdcomb").val().concat($("#dmdctr").val(),$("#baldteasof").val()))
	$("#drugcurPic").attr("src",baseURL+"assets/drug_photo/"+drugPic+".jpg?timestamp=" + new Date().getTime());
});
...
$('#formDrugPic').submit('click',function(){
	...
	success: function(data,response){
		...
		$("#drugPhoto").attr("src","assets/drug_photo/"+data['file_name']+"?timestamp=" + new Date().getTime());
	},
