...
function OPDLogList() {
    ...
    { data: "opdtime" },
    {
      render: function (data, type, row) {
        return (
          ...
          "/" +
          btoa(row.hpercode) +
          "'class='dropdown-item' data-enccode='" +
          btoa(row.enccode) +
          "' data-percode='" +
          row.hpercode +
          ...
          "' data-percode='" +
          row.hpercode +
          "'><i class='fa fa-archive'></i>&nbsp Records</a>" +
          "<a  href='" +
          baseURL +
          "DoctorsOrder/Orders/" +
          btoa(row.enccode) +
          ...
          "' data-percode='" +
          row.hpercode +
          "'><i class='fa fa-stethoscope'></i>&nbsp Order</a>" +
          "<a  href='" +
          baseURL +
          "Nurse/labResults/" +
          btoa(row.enccode) +
          ...
          "' data-percode='" +
          row.hpercode +
          "'><i class='fa fa-flask'></i>&nbsp Results</a>" +
          "<a href=javascript:void(0); class='dropdown-item' id='btnRefer' data-enccode='" +
          row.enccode +
          "' data-percode='" +
          row.hpercode +
          "'><i class='fa fa-columns'></i>&nbsp Refer</a>" +
          "</div>" +
          "<button type='button' data-enccode='" +
          btoa(row.enccode) +
          "' class='btn btn-outline-success btn-sm' id ='btnInformation'><i class='fa fa-user-circle fa-sm'></i></button>" +
          "</div>"
        );
      },
    },
  ...
  $("#frmFilter").on("submit", function (e) {
    ...
    /*$.ajax({
      ...
      success: function (data, response) {
        ...
        $("#txtDateRange").text(
          "Consultations Filtered:".concat(
            "FROM: ",
            dt.toLocaleDateString(),
            "     -   ",
            "TO: ",
            dt2.toLocaleDateString()
          )
        );
  ...
  opdData.on("click", "#btnInformation", function () {
    ...
    Obj.type = "POST";
    Obj.url =
      "Outpatient/OutpatientInformation/" +
...
function OPDSignatories(enctr) {
  $("#formOPDSignatories").validate({
    submitHandler: function (form) {
      ...
      var src =
        baseURL +
        "OPD_PatientProfile/Opd_PatientProfile/" +
        enctr +
        "/" +
        doctor;
      var x = (document.getElementById("CSS_pdf").src = src);
