...
function checkAssess(enccode, hpercode) {
	$.ajax({
		...
		success: function (data) {
			if (data == 0) {
				toastr.warning('Please fill out the assessment form first.', 'Warning');
				$('#MSSAddButton').attr('disabled', true);
			}
			else {
				$('#ModalAddSocialService').modal({ backdrop: 'static' }).draggable({});;
...
$("#SocialServiceTable").on("click", ".ModalSocialService", function () {
	...
	if (old == 'Y') {
		$('#SocialServicePDF').html('<embed src="' + "SocialService/AssessmentFormOld/" + enccode + "/" + hpercode + '" frameborder="2" width="100%" height="650px" id="SocialServicePDF">');
	}
	else if (newpat == 'Y') {
		$('#SocialServicePDF').html('<embed src="' + "SocialService/AssessmentFormNew/" + enccode + "/" + hpercode + '" frameborder="2" width="100%" height="650px" id="SocialServicePDF">');
	}
	else {
		$('#SocialServicePDF').html('<embed src="' + "SocialService/AssessmentFormOld/" + enccode + "/" + hpercode + '" frameborder="2" width="100%" height="650px" id="SocialServicePDF">');
	}
