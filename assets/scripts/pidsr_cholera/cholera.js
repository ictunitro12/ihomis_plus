...
function initCholera(hpercode, enccode) {
	CivilStatus();
	// uppercase();
	SelIndig();
...
function initCholeraEdit(hpercode, enccode) {
	initcheck();
	CivilStatus();
	// uppercase();
	getCholeraData(enccode, hpercode);
	$('#enccode').val(enccode);
	$('#formIden').val('update');
...
function getPatientData(enccode) {
	...
	$.ajax({
		url: baseURL + "PIDSR_Cholera/getPatientData/" + enctr,
		type: "POST",
		data: "JSON",
		processData: false,
		cache: false,
...
function getCholeraData(enccode, hpercode) {
	...
	$.ajax({
		...
		success: function (data, response) {
			...
			$('#date_firstconsult').val(setTimeLocale(obj['date_consultation']));
			$('#placefirstconsult').val(obj['place_consultation']);
			...
			var date_died = obj['date_died'];
			var date_dieds = ((date_died == "1970-01-01 00:00:00") || (date_died == null)) ? "0000-00-00 00:00:00" : setTimeLocale(date_died);
			$('#mainsourcewater option[value="' + obj['mainsourcewater'] + '"]').attr("selected", true);
			$('#mainsourcewater_others').val(obj['mainsourcewater_others']);
			$('#date_specimen').val(setTimeLocale(obj['date_specimen']));
			$('#stool_result option[value="' + obj['stool_result'] + '"]').attr("selected", true);
			...
			if (obj['mainsourcewater'] == 'O') {
				$('#mainsourcewater_others').removeAttr("disabled", true);
			}
			else {
